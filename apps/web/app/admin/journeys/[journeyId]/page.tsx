...
export default function JourneyEditorPage() {
  ...
  const handleFieldSave = async (updates: Partial<JourneyResponse>) => {
    ...
    try {
      ...
      await loadJourney();
      notify(t("journeys.editor.journeyUpdated"));
      setEditingName(false);
      setEditingDescription(false);
    } catch (error) {
      console.error("Save failed:", error);
      notify(t("journeys.editor.errorSaving"));
      throw error;
    } finally {
  ...
  const handleGeneratePhaseWithAI = async () => {
    ...
    try {
      ...
      if (parsed) {
        ...
        setError(t("journeys.editor.aiGenerationFailed"));
        notify(t("journeys.editor.aiGenerationFailed"));
      }
    } catch (err) {
  ...
  const handleDeletePhase = async (phaseId: string) => {
    ...
    try {
      ...
      await loadJourney();
      notify(t("journeys.editor.deleted"));
    } catch (err) {
  ...
  const triggerMomentGeneration = async () => {
    if (!journey) {
      setMomentsError(t("journeys.editor.momentsJourneyNotLoaded"));
      return;
    }
    if (!phaseFormData.name.trim() && !phaseFormData.description?.trim()) {
      setMomentsError(t("journeys.editor.momentsPhaseRequired"));
      return;
    }
    setMomentsError(null);
    try {
      ...
      if (!result.suggestions.length) {
        setMomentsError(t("journeys.editor.momentsNoneGenerated"));
        return;
      }
  ...
  if (loading) {
    return (
      <div style={{ padding: "2rem", textAlign: "center" }}>
        ...
        <MsqdxTypography variant="body2" sx={{ color: "text.secondary" }}>{t("journeys.editor.loading")}</MsqdxTypography>
      </div>
    );
  }
  ...
  if (error || !journey) {
    return (
      <div style={{ padding: "2rem" }}>
        <MsqdxTypography variant="body2" sx={{ color: "error.main" }}>{error || t("journeys.editor.notFound")}</MsqdxTypography>
      </div>
    );
  }
          ...
          <Box sx={{ display: "flex", flexDirection: "row", gap: "8px", flex: 1, minWidth: 0, flexWrap: "wrap", alignItems: "center" }}>
            {editingName ? (
              <Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                />
                <MsqdxButton variant="text" size="small" onClick={saveName} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.saveName")}>
                  <MsqdxIcon name="check" customSize={16} />
                </MsqdxButton>
                <MsqdxButton variant="text" size="small" onClick={() => { setEditingName(false); setNameValue(journey?.name || ""); }} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.cancel")}>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <MsqdxButton variant="text" size="small" onClick={startEditingName} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.editName")}>
            ...
            {editingDescription ? (
              <Box sx={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
                ...
                />
                <MsqdxButton variant="text" size="small" onClick={saveDescription} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.saveDescription")}>
                  <MsqdxIcon name="check" customSize={16} />
                </MsqdxButton>
                <MsqdxButton variant="text" size="small" onClick={() => { setEditingDescription(false); setDescriptionValue(journey?.description || ""); }} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.cancel")}>
                  <MsqdxIcon name="close" customSize={16} />
                </MsqdxButton>
              </Box>
            ) : (
              journey.description ? (
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
                  ...
                  <MsqdxButton variant="text" size="small" onClick={startEditingDescription} disabled={savePending} sx={{ minWidth: 28, minHeight: 28, p: 0 }} aria-label={t("journeys.editor.editDescription")}>
                ...
                <MsqdxButton variant="text" size="small" onClick={startEditingDescription} disabled={savePending} sx={{ alignSelf: "flex-start", color: "text.secondary" }}>
                  <MsqdxIcon name="add" customSize={14} /> {t("journeys.editor.addDescription")}
                </MsqdxButton>
              )
            )}
          </Box>
              ...
              >
                <Box>
                  <MsqdxTypography variant="caption" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.05em", display: "block", mb: 0.25 }}>
                    {t("journeys.editor.metadataType")}
                  </MsqdxTypography>
                ...
                {typeof journey.validation_score === "number" && (
                  <Box sx={{ borderLeft: "1px solid", borderColor: "divider", pl: 1 }}>
                    <MsqdxTypography variant="caption" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.05em", display: "block", mb: 0.25 }}>
                      {t("journeys.editor.validationScore")}
                    </MsqdxTypography>
                ...
                <Box sx={{ borderLeft: "1px solid", borderColor: "divider", pl: 1 }}>
                  <MsqdxTypography variant="caption" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.05em", display: "block", mb: 0.25 }}>
                    {t("journeys.editor.tracking")}
                  </MsqdxTypography>
                  <MsqdxTypography variant="body2" weight="medium">{journey.tracking_enabled ? t("journeys.editor.enabled") : t("journeys.editor.disabled")}</MsqdxTypography>
                </Box>
                <Box sx={{ borderLeft: "1px solid", borderColor: "divider", pl: 1 }}>
                  <MsqdxTypography variant="caption" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.05em", display: "block", mb: 0.25 }}>
                    {t("journeys.editor.created")}
                  </MsqdxTypography>
                  <MsqdxTypography variant="body2" weight="medium">{formatDate(journey.created_at)}</MsqdxTypography>
                </Box>
                <Box sx={{ borderLeft: "1px solid", borderColor: "divider", pl: 1 }}>
                  <MsqdxTypography variant="caption" sx={{ color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.05em", display: "block", mb: 0.25 }}>
                    {t("journeys.editor.updated")}
                  </MsqdxTypography>
        ...
        <div className="msqdx-glass-detail__section">
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, mb: 2 }}>
            <MsqdxTypography variant="h5" weight="semibold">
              {t("journeys.editor.phasesCount", { count: journey.phases.length })}
            </MsqdxTypography>
          ...
          {journey.phases.length === 0 && !phaseFormExpanded ? (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5, mb: 2 }}>
                ...
                {phaseFormExpanded ? (
                  <MsqdxCard className="msqdx-glass-journey-timeline-card" variant="flat" borderRadius="button" sx={{ minWidth: 380, p: 2, borderColor: THEME_ACCENT.borderColor }}>
                    <Box component="form" onSubmit={handleSubmitPhase} aria-label="Create new phase" sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          <MsqdxTypography variant="caption" sx={{ color: "text.secondary" }}>{editingPhaseId ? t("journeys.editor.editPhase") : t("journeys.editor.newPhase")}</MsqdxTypography>
                          <MsqdxTypography variant="h6" weight="semibold">{editingPhaseId ? t("journeys.editor.updateDetails") : t("journeys.editor.defineNext")}</MsqdxTypography>
                        </Box>
                        {editingPhaseId && (
                          <MsqdxButton variant="text" size="small" onClick={() => { setPhaseFormExpanded(false); setEditingPhaseId(null); setPhaseFormData({ name: "", description: "", phase_order: journey.phases.length + 1, expected_duration_min: undefined, expected_duration_max: undefined, duration_unit: "minutes", expected_emotion: undefined, emotion_intensity: undefined }); setMomentDrafts([]); setMomentsError(null); setError(null); }} disabled={addingPhase} aria-label={t("journeys.editor.cancelEditing")}><MsqdxIcon name="close" customSize={18} /></MsqdxButton>
                        )}
                      </Box>
                      {error && <MsqdxTypography variant="body2" color="error">Error: {error}</MsqdxTypography>}
                      <MsqdxFormField label={t("journeys.editor.phaseNameLabel")} value={phaseFormData.name} onChange={(e) => setPhaseFormData({ ...phaseFormData, name: e.target.value })} placeholder={t("journeys.editor.placeholderPhaseName")} required disabled={addingPhase} fullWidth size="small" sx={FORM_FIELD_ACCENT_SX} />
                      ...
                      <Box>
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                          <Box>
                            <MsqdxTypography variant="caption" weight="semibold">{t("journeys.editor.journeyMoments")}</MsqdxTypography>
                            <MsqdxTypography variant="caption" sx={{ color: "text.secondary", display: "block" }}>{t("journeys.editor.journeyMomentsHint")}</MsqdxTypography>
                          </Box>
                          <Box sx={{ display: "flex", gap: 0.5 }}>
                            <MsqdxGlassAiButton templates={[{ id: "journey.moments", label: t("journeys.editor.aiSuggestion"), maxSuggestions: 4 }]} onClick={triggerMomentGeneration} disabled={!journey || addingPhase || aiAssistLoading} loading={aiAssistLoading} size="small" title={t("journeys.editor.aiSuggestion")} />
                            <MsqdxButton variant="outlined" size="small" onClick={addMomentDraft} disabled={addingPhase} startIcon={<MsqdxIcon name="add_circle" customSize={14} />}>{t("journeys.editor.addMoment")}</MsqdxButton>
                          </Box>
                        </Box>
                        {momentsError && <MsqdxTypography variant="caption" color="error" sx={{ display: "block", mb: 1 }}>{momentsError}</MsqdxTypography>}
                        {momentDrafts.length === 0 ? (
                          <MsqdxTypography variant="body2" sx={{ color: "text.secondary", py: 2 }}>{t("journeys.editor.noMoments")}</MsqdxTypography>
                        ) : (
                  ...
                  >
                    ...
                    >
                      ...
                      <MsqdxTypography variant="h6" weight="semibold" sx={{ mb: 0.5 }}>{t("journeys.editor.addPhase")}</MsqdxTypography>
                      <MsqdxTypography variant="body2" sx={{ color: "text.secondary" }}>
                        {t("journeys.editor.addPhaseCardHint")}
                      </MsqdxTypography>
                    </Box>
              ...
              <MsqdxTypography variant="caption" sx={{ color: "text.secondary", display: "block", mt: 1 }} component="div" aria-live="polite">
                {t("journeys.editor.phaseOf", { current: activePhaseIndex + 1, total: journey.phases.length })}
              </MsqdxTypography>
            </Box>
          )}
        </div>
