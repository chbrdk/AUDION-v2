...
| Entity header | `components/admin/section-shell/msqdx-glass-section-entity-header.tsx` — optional `entityCornerAccent`: dekoratives `MsqdxCornerBox` **oben links** am Header (`cutdown-b` oben links + unten rechts, `rounded` oben rechts; Radius wie Subnav-Dock) |
