...
- **Server-seitig (Persona-API):** Wenn `PLEXON_API_BASE_URL` + `PLEXON_SERVICE_SECRET` gesetzt sind und der User **`plexon_user_id`** hat, holt die API bei **POST /projects**, **Easy-Setup** und **POST /projects/{id}/plexon-mirror** die fehlende **`platform_company_id`** per **`GET {PLEXON}/api/services/profile?user_id=…`** nach (Feld `default_platform_company_id` im User-Objekt). So funktionieren Anlage und PLEXON-Retry auch ohne expliziten Client-Kontext, sobald PLEXON das Profil liefert.
