...
| `UX_JOURNEY_VIDEO_SCENE_END_OFFSETS` | no | **Default `1` (ON).** Treats `videoOffsetSec[N]` as the **end** of step N's action — which is what the agent's history-watcher actually records (history grows after the action commits). Scene N then spans `[offset[N-1] \| 0, offset[N])`, i.e. the slice that visibly shows step N's cursor / click / scroll / page settle. With `0` (legacy), scene N spans `[offset[N], offset[N+1])`, which previously caused an off-by-one between voice-over (step N's reasoning) and visible action (step N+1). Keep `1`; flip to `0` only to A/B against the old behaviour. |
