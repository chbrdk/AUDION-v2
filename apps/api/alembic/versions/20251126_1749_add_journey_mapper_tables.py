...
def upgrade() -> None:
    # Create ENUM types
    op.execute("""
        CREATE TYPE journey_element_type AS ENUM (
            'action', 'thought', 'feeling', 'touchpoint', 
            'pain_point', 'opportunity', 'question', 'quote'
        )
    """)
    
    op.execute("""
        CREATE TYPE journey_metric_type AS ENUM (
            'sessions', 'users', 'page_views', 'bounce_rate', 
            'time_on_page', 'scroll_depth', 'engagement_rate', 
            'conversion_rate', 'form_submissions', 'cta_clicks', 
            'cta_click_rate', 'rage_clicks', 'u_turns', 
            'error_clicks', 'leads', 'opportunities', 'revenue'
        )
    """)
    
    op.execute("""
        CREATE TYPE journey_comparison_operator AS ENUM (
    ...
    op.execute("""
        CREATE TYPE journey_measurement_status AS ENUM (
            'good', 'warning', 'critical', 'no_data'
        )
    """)
    
    op.execute("""
        CREATE TYPE journey_insight_type AS ENUM (
            'confirmation', 'contradiction', 'discovery', 'anomaly'
        )
    """)
    
    op.execute("""
        CREATE TYPE journey_creation_mode AS ENUM (
    ...
    op.execute("""
        CREATE TYPE journey_insight_status AS ENUM (
