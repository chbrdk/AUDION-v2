...
export function UdgGlassKnowledgeExplorer({ targetGroupId }: KnowledgeExplorerProps) {
  ...
  if (!targetGroupId) {
    return (
      <div className="udg-glass-knowledge-explorer">
        <h3 style={{ fontSize: "1.5rem", fontWeight: 100, marginBottom: "2rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>
          Knowledge Explorer
        </h3>
        <p className="udg-glass-empty">Please select a Target Group to explore knowledge.</p>
      </div>
    );
  }
  ...
  return (
    <div className="udg-glass-knowledge-explorer">
      ...
      {error && (
        <div className="udg-glass-error" style={{ padding: "0.75rem", marginBottom: "1rem", borderRadius: "0", border: "1px solid var(--color-secondary-dx-pink)", backgroundColor: "var(--color-secondary-dx-pink-tint)" }}>
          <strong>Error:</strong> {error}
        </div>
      )}

      {loading && (
        <p className="udg-glass-empty">Loading chunks and generating clusters...</p>
      )}

      {!loading && !error && chunks.length === 0 && (
        <p className="udg-glass-empty">No chunks available for clustering. Upload documents to explore knowledge.</p>
      )}
...
function KnowledgeListView({
  ...
  return (
    <div className="udg-glass-knowledge-list">
      {chunksByCluster.map((cluster) => (
        <div key={cluster.id} className="udg-glass-detail__section">
          ...
          <div className="udg-glass-list">
            {cluster.chunks.slice(0, 12).map((chunk) => (
              <div
                key={chunk.id}
                className={`udg-glass-list-item ${selectedChunk?.id === chunk.id ? "is-active" : ""}`}
            ...
            {cluster.chunks.length > 12 && (
              <p className="udg-glass-muted" style={{ fontSize: "0.8125rem", textAlign: "center", padding: "0.75rem", margin: 0 }}>
...
function ChunkDetailPanel({ chunk, similarChunks, onClose }: ChunkDetailPanelProps) {
  return (
    <div
      className="udg-glass-chunk-detail-panel"
      style={{
        position: "fixed",
        right: "0",
        top: "0",
    ...
    >
      ...
      <div
        className="udg-glass-chunk-detail-panel__content"
        style={{
          padding: "1rem",
          overflowY: "auto",
          flex: 1,
      ...
      >
        <div className="udg-glass-detail__section" style={{ marginBottom: "1rem" }}>
        ...
        {similarChunks.length > 0 && (
          <div className="udg-glass-similar-chunks">
