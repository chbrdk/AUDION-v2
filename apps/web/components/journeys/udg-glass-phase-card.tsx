...
type UdgGlassJourneyPhaseCardProps = {
  phase: JourneyPhase;
  index: number;
  isActive: boolean;
  journeyId: string;
...
export const UdgGlassJourneyPhaseCard = ({
  phase,
  index,
  isActive,
  journeyId,
  ...
  if (isEditing) {
    return (
      <article
        className={clsx("udg-glass-journey-phase", "--editing", isActive && "--active")}
        data-phase-index={index}
        aria-label={`Phase ${index + 1}: ${formData.name}`}
      >
        <div className="udg-glass-journey-phase__header">
          <div className="udg-glass-journey-phase__badge">{index + 1}</div>
          <div style={{ flex: 1, position: "relative" }}>
            <p className="udg-glass-journey-phase__eyebrow">Phase</p>
            <input
              type="text"
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="udg-glass-journey-phase__title-input"
              placeholder="Phase name"
              disabled={saving}
              style={{ paddingRight: "2.5rem" }}
            />
            <UdgGlassAiFieldButton
        ...
        <div className="udg-glass-journey-phase__section" style={{ position: "relative" }}>
          <p className="udg-glass-journey-phase__section-label">Focus</p>
          <textarea
            value={formData.description}
            onChange={(e) => setFormData({ ...formData, description: e.target.value })}
            className="udg-glass-journey-phase__description-input"
        ...
        <div className="udg-glass-journey-phase__form-grid">
          ...
          <label className="udg-glass-journey-phase__form-field" style={{ position: "relative" }}>
  ...
  return (
    <article
      className={clsx("udg-glass-journey-phase", isActive && "--active")}
      data-phase-index={index}
    ...
    >
      ...
      {chips.length > 0 && (
        <div className="udg-glass-journey-phase__chips">
          {chips.map((chip) => (
            <span key={chip.label} className="udg-glass-journey-phase__chip">
      ...
      <div className="udg-glass-journey-phase__section">
        <p className="udg-glass-journey-phase__section-label">Focus</p>
        <p className="udg-glass-journey-phase__description">
      ...
      <div className="udg-glass-journey-phase__section">
        ...
        {highlightedElements.length === 0 ? (
          <p className="udg-glass-journey-phase__empty">No elements yet. Map touchpoints, thoughts or feelings.</p>
        ) : (
          <ul className="udg-glass-journey-phase__list">
            {highlightedElements.map((element) => (
              <li key={element.id}>
                <span className="udg-glass-journey-phase__list-icon">
                  <MaterialSymbol icon={elementIconMap[element.element_type] ?? "trip"} fontSize={14} />
                </span>
                <div>
                  <p className="udg-glass-journey-phase__list-label">{element.element_type.replace("_", " ")}</p>
                  <p className="udg-glass-journey-phase__list-value">{element.content}</p>
                </div>
              </li>
            ))}
          </ul>
...
export type UdgGlassPhaseCardProps = {
  phase: JourneyPhase;
  index: number;
  isSelected?: boolean;
  onSelect?: () => void;
...
export const UdgGlassPhaseCard = ({
  phase,
  index,
  isSelected,
  onSelect,
}: UdgGlassPhaseCardProps) => {
