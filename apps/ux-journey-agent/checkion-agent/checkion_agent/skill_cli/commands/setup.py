...
async def run_checks() -> dict[str, Any]:
	...
	# Package check
	try:
		import checkion_agent

		checks['checkion_agent_package'] = {
			'status': 'ok',
			'message': f'browser-use {checkion_agent.__version__}'
			if hasattr(checkion_agent, '__version__')
...
async def validate_setup() -> dict[str, Any]:
	...
	# Check imports
	try:
		import checkion_agent  # noqa: F401

		results['checkion_agent_import'] = 'ok'
	except ImportError:
		results['checkion_agent_import'] = 'failed'
