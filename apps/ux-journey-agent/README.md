...
| `UX_JOURNEY_LLM_REPAIR_JSON` | no | When **`1`**, wraps primary and fallback chat models in a thin post-processor that (a) extracts the first balanced JSON object if the model appended junk after the closing `}` (``trailing characters`` / ``json_invalid``), and (b) if ``action`` is a string that contains a JSON array, parses it into a real list (fixes Pydantic ``list_type`` on ``action``). **Default `0`**. Best-effort only — disable if a browser-use upgrade changes the schema and you see new failures. |
