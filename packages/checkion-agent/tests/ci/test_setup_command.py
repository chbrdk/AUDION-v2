...
async def test_run_checks():
	...
	assert 'checkion_agent_package' in checks
	assert checks['checkion_agent_package']['status'] in ('ok', 'error')
...
def test_plan_actions_no_actions_needed():
	...
	checks = {
		'checkion_agent_package': {'status': 'ok'},
		'browser': {'status': 'ok'},
	}
...
async def test_validate_setup():
	...
	assert 'checkion_agent_import' in results
