...
def upgrade() -> None:
    ...
    print("Running patch migration 20260127_fix_obj_key...", flush=True)
    ...
    if bind.dialect.name == 'postgresql':
        try:
            ...
            print("20260127_fix_obj_key: ALTER TABLE done.", flush=True)
        except Exception as e:
            print(f"20260127_fix_obj_key ERROR: {e!r}", flush=True)
            raise
    else:
...
def downgrade() -> None:
    bind = op.get_bind()
    print("Reverting patch migration 20260127_fix_obj_key...", flush=True)
    if bind.dialect.name == 'postgresql':
