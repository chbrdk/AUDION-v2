...
- **Journeys** view: after selecting a journey, choose **phase**, **persona**, optional **target group**, then:
  - **Build screen prompt only** — calls CREATION `POST /api/v1/journey-screen-brief` and fills the **Prompt → Website → Figma** textarea on the same screen.
...
- **`JourneysPanel`** must not use **`height: 100%`** on its root when stacked above `PromptSiteToFigmaPanel` — that pattern steals the full viewport height inside nested flex layouts and causes “overlay” / dead-click behaviour in the sandboxed UI.
