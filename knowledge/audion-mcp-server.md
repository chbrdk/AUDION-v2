...
- **Persona Chat tools** additionally call **chat-api** (port 8001) via `CHAT_API_URL`.
...
| `CHAT_API_URL` | MCP server | Chat API base for `audion.chat_*` (e.g. `http://audion-chat-api:8001`). |
...
**API coverage audit:** [audion-mcp-api-coverage.md](./audion-mcp-api-coverage.md) (MCP vs. FastAPI routes, gaps, changelog).
...
- Run with `AUDION_API_URL`, `AUDION_API_TOKEN`, and `CHAT_API_URL` (for chat tools).
...
- **Chat tools missing / only `audion.chat_status`**: Set `CHAT_API_URL` to the internal chat-api service URL.
