...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('checkion_agent')
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('checkion_agent.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('checkion_agent.'):
					# For other checkion_agent modules, use the last part
	...
	# Configure checkion_agent logger
	...
	checkion_agent_logger.addHandler(console)
	for handler in file_handlers:
		checkion_agent_logger.addHandler(handler)
	checkion_agent_logger.setLevel(effective_log_level)
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['checkion_agent.agent', 'checkion_agent.tools']:
	...
	for name in ['bubus', 'checkion_agent.browser.session']:
		logger = logging.getLogger(name)
