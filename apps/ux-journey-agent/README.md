...
| `UX_JOURNEY_LLM_FALLBACK` | no | When **`1`** (default), and both `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are configured, the agent passes the **other** provider as `fallback_llm` to browser-use. Recent browser-use versions retry on the fallback for HTTP errors *and* AgentOutput validation failures (e.g. `action` returned as JSON string instead of a list). Set to `0` to disable. |
