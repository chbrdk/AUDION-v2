...
def test_install_command_help():
	...
	result = subprocess.run(
		[sys.executable, '-m', 'checkion_agent.skill_cli.main', '--help'],
		capture_output=True,
		text=True,
	)
...
def test_init_subcommand_help():
	...
	result = subprocess.run(
		[sys.executable, '-m', 'checkion_agent.skill_cli.main', 'init', '--help'],
		capture_output=True,
		text=True,
	)
	assert result.returncode == 0
...
def test_init_list_templates():
	...
	result = subprocess.run(
		[sys.executable, '-m', 'checkion_agent.skill_cli.main', 'init', '--list'],
		capture_output=True,
		text=True,
	)
	assert result.returncode == 0
