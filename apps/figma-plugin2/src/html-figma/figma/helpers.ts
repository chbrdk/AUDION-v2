...
/** Plugin-internal keys on layer JSON (e.g. after peelFrameLayoutMeta). Must not be copied onto Figma nodes (not extensible). */
function isHtmlFigmaInternalLayerKey(key: string): boolean {
    return key.startsWith('__htmlFigma') || key.startsWith('__dom');
}

export function assign(a: BaseNode & AnyStringMap, b: AnyStringMap) {
    for (const key in b) {
        if (isHtmlFigmaInternalLayerKey(key)) {
            continue;
        }
        const value = b[key];
