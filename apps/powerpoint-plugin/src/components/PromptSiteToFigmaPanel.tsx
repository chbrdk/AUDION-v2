...
export interface PromptSiteToFigmaPanelProps {
  ...
  prefillToken?: number;
  /** When set (e.g. after journey screen brief), sent with generate for Figma section metadata. */
...
export function PromptSiteToFigmaPanel({
  ...
  const handleSubmit = () => {
    ...
    const concepts =
      journeyPipeline &&
      Array.isArray(journeySectionConcepts) &&
      journeySectionConcepts.length > 0
        ? journeySectionConcepts
        : undefined;
    onGenerate(trimmed, viewport, lib, mode, concepts);
  };
