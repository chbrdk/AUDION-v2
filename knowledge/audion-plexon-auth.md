...
- **Nachziehen:** `POST /projects/{project_id}/plexon-mirror` (Owner/Admin) ruft dieselbe PLEXON-Registrierung erneut auf, wenn `platform_project_id` noch fehlt (optionaler JSON-Body `{ "platform_company_id": "…" }`, falls das Projekt noch keine hat). Ist bereits registriert, liefert die API `plexon_mirror_status: already_synced`.
