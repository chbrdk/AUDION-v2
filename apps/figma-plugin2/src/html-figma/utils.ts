...
import { getImagePaintsFromLayer } from './figma/sanitize-paints';
...
export async function traverseAsync<T>(
    ...
    if (layer) {
        ...
        if (hasChildren(layer)) {
            // @ts-ignore — iterate in reverse DOM order without mutating `children`
            const kids = (layer as { children: T[] }).children;
            for (let i = kids.length - 1; i >= 0; i--) {
                await traverseAsync(kids[i] as T, cb, layer);
            }
        }
    }
}
...
/** Plain layer JSON from html-to-figma / API. */
export type LayerWithOptionalFills = Record<string, unknown> & {
    fills?: unknown;
};

/** IMAGE paints from `fills` and `backgrounds` (frames may use either). */
export function getImageFills(layer: unknown): false | any[] {
