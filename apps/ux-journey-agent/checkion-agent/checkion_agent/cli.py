...
if 'init' in sys.argv:
	from checkion_agent.init_cmd import INIT_TEMPLATES
	from checkion_agent.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'checkion_agent_{template}.py'
...
from checkion_agent import Agent, Controller
from checkion_agent.agent.views import AgentSettings
from checkion_agent.browser import BrowserProfile, BrowserSession
from checkion_agent.logging_config import addLoggingLevel
from checkion_agent.telemetry import CLITelemetryEvent, ProductTelemetry
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('checkion_agent.'):
		...
		# Configure checkion_agent logger and all its sub-loggers
		checkion_agent_logger = logging.getLogger('checkion_agent')
		checkion_agent_logger.propagate = False  # Don't propagate to root logger
		checkion_agent_logger.handlers = [log_handler]  # Replace any existing handlers
		checkion_agent_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['checkion_agent.Agent', 'checkion_agent.controller', 'checkion_agent.agent', 'checkion_agent.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'checkion_agent.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('checkion_agent.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('checkion_agent.app')
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent checkion_agent from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('checkion_agent.startup')
	...
	try:
		...
		try:
			from checkion_agent.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from checkion_agent.sync.auth import DeviceAuthClient
	...
	try:
		...
		from checkion_agent.sync.service import CloudSync
		...
		try:
			...
			from checkion_agent.utils import create_task_with_error_handling
		...
		if task_id and sync_service:
			try:
				from checkion_agent.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: checkion_agent_<template>.py)')
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from checkion_agent.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'checkion_agent_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: checkion_agent_<template>.py)',
)
@click.option(
	'--force',
	'-f',
