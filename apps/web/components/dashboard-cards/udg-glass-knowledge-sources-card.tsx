...
export type UdgGlassKnowledgeSourcesCardProps = {
...
export const UdgGlassKnowledgeSourcesCard = ({
  detail,
  knowledgeForm,
...
}: UdgGlassKnowledgeSourcesCardProps) => {
    ...
    >
      <UdgGlassDashboardCardSection>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            {detail.documents.some((doc) => doc.ingestionStatus === "pending" || doc.ingestionStatus === "processing") && (
              <span className="udg-glass-muted" style={{ fontSize: "0.75rem", display: "flex", alignItems: "center", gap: "4px" }}>
        ...
        {detail.documents.length === 0 && (
          <p className="udg-glass-muted" style={{ margin: 0 }}>No documents uploaded.</p>
        )}
        {detail.documents.length > 0 && (
          <ul className="udg-glass-card-list" style={{ marginTop: "0.5rem" }}>
            {detail.documents.map((doc) => {
              const ingestionChip = doc.ingestionStatus
                ? doc.ingestionStatus === "completed"
                  ? { label: "Indexed", className: "udg-glass-chip --success" }
                  : doc.ingestionStatus === "processing"
                    ? { label: `Processing ${doc.ingestionProgress ? Math.round(doc.ingestionProgress) : 0}%`, className: "udg-glass-chip --processing" }
                    : doc.ingestionStatus === "failed"
                      ? { label: "Error", className: "udg-glass-chip --error" }
                      : { label: "Pending", className: "udg-glass-chip --pending" }
                : null;
              return (
                <li key={doc.id}>
                  <div>
                  ...
                  {doc.insightSummary && <p className="udg-glass-code-block">{doc.insightSummary}</p>}
                </li>
              );
            })}
          </ul>
      ...
      <UdgGlassDashboardCardSection title="Knowledge Base">
        <form className="udg-glass-field-grid" onSubmit={onKnowledgeSubmit}>
          ...
          <div className="udg-glass-field">
            <button className="udg-glass-button" type="submit" disabled={knowledgePending}>
              <MaterialSymbol icon="lightbulb" fontSize={16} /> {knowledgePending ? "Saving..." : "Add knowledge"}
            </button>
          </div>
        </form>
        {detail.knowledge.length === 0 && (
          <p className="udg-glass-muted" style={{ marginTop: "1rem", marginBottom: 0 }}>
            No knowledge entries yet.
          </p>
        )}
        {detail.knowledge.length > 0 && (
          <ul className="udg-glass-card-list" style={{ marginTop: "1rem" }}>
            {detail.knowledge.map((entry) => (
              <li key={entry.id}>
                ...
                <p className="udg-glass-code-block">{entry.content}</p>
              </li>
            ))}
          </ul>
        )}
      ...
      <UdgGlassDashboardCardSection title="Sources">
        {detail.sources.length === 0 && (
          <p className="udg-glass-muted" style={{ margin: 0 }}>No sources linked.</p>
        )}
        {detail.sources.length > 0 && (
          <ul className="udg-glass-sources" style={{ marginTop: "0.5rem", marginBottom: 0 }}>
      ...
      {detail.insights && (
        <UdgGlassDashboardCardSection title="Insights">
          <dl className="udg-glass-meta-grid" style={{ marginTop: "0.5rem", marginBottom: "0.5rem" }}>
            <div>
              <dt>Chunks</dt>
