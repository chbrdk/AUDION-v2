...
/**
 * Public moodboard endpoint for shared chat links. No auth required.
 * Proxies to persona backend GET /personas/{id}/moodboards/public?project_id=xxx
 */
export async function GET(request: NextRequest, context: PersonaContext) {
  ...
  const target = `${base}/personas/${personaId}/moodboards/public?project_id=${encodeURIComponent(projectId)}`;
  ...
  try {
    ...
    console.error("[api/share/persona/moodboard] Fetch failed:", error);
    return NextResponse.json({ error: "Failed to load moodboard" }, { status: 502 });
  }
}
