...
- PLEXON ergänzt im Service-Profil optional **`default_platform_company_id`**: die `companies.id` der **ältesten** Zeile in `company_users` für diesen Nutzer. AUDION merged das Feld in den User und nutzt es als letzten Fallback in `resolvePlatformCompanyIdForApi` (nach URL-Parameter, `sessionStorage`, `NEXT_PUBLIC_DEFAULT_PLATFORM_COMPANY_ID`), damit **POST /projects** und Easy-Setup ohne Deep-Link eine `platform_company_id` senden können. Nach **login / register / GET /api/auth/me** schreibt die Web-App diesen Wert zusätzlich in **`sessionStorage`** (`audion_platform_company_id`), sobald noch kein Tab-Kontext gesetzt ist — damit bleibt der Kontext über Navigationen erhalten.
- **Server-seitig (Persona-API):** Wenn `PLEXON_API_BASE_URL` + `PLEXON_SERVICE_SECRET` gesetzt sind, holt die API bei **POST /projects**, **Easy-Setup** und **POST /projects/{id}/plexon-mirror** eine fehlende **`platform_company_id`** per **`GET {PLEXON}/api/services/profile?user_id=…`** nach; liefert die Antwort kein `default_platform_company_id`, wird **`GET …?email=…`** (E-Mail des Persona-Users) als zweiter Versuch genutzt — analog zu CHECKION. Die Web-App sendet beim **PLEXON-Retry** dieselbe aufgelöste Kontext-ID wie beim Projekt-Anlegen (URL → `sessionStorage` → `NEXT_PUBLIC_DEFAULT_PLATFORM_COMPANY_ID` → `default_platform_company_id` aus `/api/auth/me`), damit der Request nicht mit leerem Body läuft, falls das Profil vom API-Container aus anders erreichbar ist als vom Browser.
- **Plattform-Projekt in PLEXON:** Nur die **AUDION-API** (Persona-Backend), nicht die Web-App, ruft nach dem Anlegen `POST {PLEXON_API_BASE_URL}/api/platform/provisioning/audion-project-origin` auf (mit `PLEXON_SERVICE_SECRET`). Ohne `PLEXON_API_BASE_URL` + Secret auf dem **API-Container** legt PLEXON keinen Eintrag an — im PLEXON-Dashboard (Plattform-Insights) erscheint dann nichts, obwohl AUDION die `platform_company_id` speichern kann. Die API-Antwort von **POST /projects** enthält `plexon_mirror_status`: `completed` = Aufruf ok, `skipped_no_env` / `skipped_no_plexon_user` = kein Eintrag in PLEXON.
- **Nachziehen:** `POST /projects/{project_id}/plexon-mirror` (Owner/Admin) ruft dieselbe PLEXON-Registrierung erneut auf, wenn `platform_project_id` noch fehlt (optionaler JSON-Body `{ "platform_company_id": "…" }`, falls das Projekt noch keine hat und das Profil keine liefert). Ist bereits registriert, liefert die API `plexon_mirror_status: already_synced`.
