...
checkion_agent/skill_cli/requirements-cli.txt.
"""
...
REQUIREMENTS_CLI = REPO_ROOT / 'checkion_agent' / 'skill_cli' / 'requirements-cli.txt'
...
def test_cli_entry_point(lite_venv):
	...
	result = subprocess.run(
		[str(lite_venv), '-m', 'checkion_agent.skill_cli.main', '--help'],
		capture_output=True,
		text=True,
		timeout=30,
	)
...
def test_daemon_imports(lite_venv):
	...
	result = _import_check(lite_venv, 'from checkion_agent.skill_cli.daemon import main')
...
def test_browser_session_imports(lite_venv):
	...
	result = _import_check(lite_venv, 'from checkion_agent.browser.session import BrowserSession')
