...
if _get_subcommand() == 'cloud':
	...
	if cloud_idx + 1 < len(sys.argv) and sys.argv[cloud_idx + 1] in ('--help', '-h'):
		from checkion_agent.skill_cli.commands.cloud import handle_cloud_command
...
def ensure_daemon(
	...
	*,
	...
	cmd = [
		sys.executable,
		'-m',
		'checkion_agent.skill_cli.daemon',
		'--session',
		session,
	]
	if headed:
...
def _migrate_legacy_files() -> None:
	...
	from checkion_agent.skill_cli.utils import migrate_legacy_paths
...
def main() -> int:
	...
	if args.command == 'profile':
		from checkion_agent.skill_cli.profile_use import run_profile_use
	...
	if args.command == 'setup':
		from checkion_agent.skill_cli.commands import setup
	...
	if args.command == 'doctor':
		from checkion_agent.skill_cli.commands import doctor
	...
	if args.command == 'tunnel':
		from checkion_agent.skill_cli import tunnel
	...
	if args.connect:
		from checkion_agent.skill_cli.utils import discover_chrome_cdp_url
