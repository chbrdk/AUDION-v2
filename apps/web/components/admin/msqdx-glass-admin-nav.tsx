...
export type MsqdxGlassAdminNavProps = {
  open: boolean;
  onClose: () => void;
  currentPath: string;
...
export const MsqdxGlassAdminNav = ({ open, onClose, currentPath, themeMode, onToggleTheme }: MsqdxGlassAdminNavProps) => {
  ...
  return (
    <>
      ...
      <Box
        component="nav"
        className="msqdx-glass-admin-nav"
          sx={{
            ...
            return (
              <ListItem key={item.path} disablePadding sx={{ width: "100%", display: "flex", justifyContent: isExpanded ? "stretch" : "center" }} suppressHydrationWarning>
                <ListItemButton
                  ...
                  className={clsx("msqdx-glass-admin-nav-item", active && "--active")}
