...
if TYPE_CHECKING:
	from checkion_agent.skills.views import Skill

from dotenv import load_dotenv

from checkion_agent.agent.cloud_events import (
...
from checkion_agent.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from checkion_agent.tokens.service import TokenCost
...
from checkion_agent import Browser, BrowserProfile, BrowserSession
from checkion_agent.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from checkion_agent.agent.gif import create_history_gif
...
from checkion_agent.browser.events import _get_timeout
from checkion_agent.browser.session import DEFAULT_BROWSER_PROFILE
from checkion_agent.browser.views import BrowserStateSummary
from checkion_agent.config import CONFIG
from checkion_agent.dom.views import DOMInteractedElement, MatchLevel
from checkion_agent.filesystem.file_system import FileSystem
from checkion_agent.observability import observe, observe_debug
from checkion_agent.telemetry.service import ProductTelemetry
from checkion_agent.telemetry.views import AgentTelemetryEvent
from checkion_agent.tools.registry.views import ActionModel
from checkion_agent.tools.service import Tools
from checkion_agent.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_checkion_agent_version,
	get_checkion_agent_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from checkion_agent.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from checkion_agent import ChatBrowserUse
		...
		if skill_service is not None:
			...
			from checkion_agent.skills import SkillService
		...
		self.agent_directory = base_tmp / f'checkion_agent_agent_{self.id}_{timestamp}'
		...
		self._set_checkion_agent_version_and_source(source)
		...
		from checkion_agent.llm.anthropic.chat import ChatAnthropic

		is_anthropic = isinstance(self.llm, ChatAnthropic)

		# Check if model is a browser-use fine-tuned model (uses simplified prompts)
		is_checkion_agent_model = 'browser-use/' in self.llm.model.lower()
		...
		self._message_manager = MessageManager(
			...
			system_message=SystemPrompt(
				...
				is_checkion_agent_model=is_checkion_agent_model,
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'checkion_agent.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from checkion_agent.screenshots.service import ScreenshotService
	...
	def _set_checkion_agent_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the browser-use package"""
		# Use the helper function for version detection
		version = get_checkion_agent_version()
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from checkion_agent.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_checkion_agent_version()
	...
	async def run(
		...
		from checkion_agent.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from checkion_agent.agent.views import RerunSummaryAction
		...
		from checkion_agent.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
	...
	async def _execute_ai_step(
		...
		"""
		from checkion_agent.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from checkion_agent.llm.messages import SystemMessage, UserMessage
		from checkion_agent.utils import sanitize_surrogates
		...
		try:
			from checkion_agent.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from checkion_agent.agent.variable_detector import detect_variables_in_history
