...
| `UX_JOURNEY_CLAUDE_MODEL` | no | Claude model (default `claude-sonnet-4-6` — Anthropic's "best speed/intelligence balance" Sonnet, ~25% faster TTFT than `claude-sonnet-4-20250514` while still solid on browser-use's strict structured-output schema). **Avoid:** Haiku 4.5 (`claude-haiku-4-5-20251001`) — 2–3× faster per step but frequently emits AgentOutput missing the required `action` field, causing browser-use to halt after 5 consecutive validation failures. `fallback_llm` only catches HTTP errors, not Pydantic validation, so Haiku is NOT production-ready with browser-use. |
