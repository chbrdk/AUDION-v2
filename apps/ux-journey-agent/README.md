...
| `UX_JOURNEY_LIVE_STEP_FRAMES` | no | **Default `0` (OFF).** Phase 4 hook — when **`1`**, the fork's `on_screenshot` callback ALSO pushes the per-step base64-PNG into the live-frame cache used by `GET /run/{jobId}/live` (in addition to incrementing the visibility counter that always runs). Off by default because the legacy live stream advertises `image/jpeg`; flipping this on works in browsers (they sniff actual bytes) but the MJPEG endpoint will be technically wrong. Use this knob if you want crisp, full-resolution per-step frames in the live preview and don't mind the content-type drift; otherwise leave it off and rely on the existing CDP-JPEG polling loop. |
