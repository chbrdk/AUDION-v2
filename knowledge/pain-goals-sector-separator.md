...
- **Bleed**: Any `.msqdx-glass-pain-goals-sector-separator` inside `.msqdx-glass-section-workspace__dock-shell` uses negative `margin-inline` and expanded `width` to cancel `--msqdx-section-workspace-dock-padding` (matches dock `px` / `py` from `SECTION_WORKSPACE_DOCK_PADDING`)
