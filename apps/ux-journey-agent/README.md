...
| `UX_JOURNEY_VIDEO_DYNAMIC_PACING` | no | **Default `1` (ON).** Per-scene pacing instead of a uniform `setpts` over the whole clip: each step's segment is sliced at `videoOffsetSec`, then time-stretched (or compressed) so its **output duration ≈ TTS clip length + `UX_JOURNEY_VIDEO_SCENE_VOICE_PAD_SEC`**. Five-step runs go from ~50 minutes (uniform `setpts=32*PTS`) to ~30–60 seconds (sum of voice clip lengths). Steps without voice fall back to `UX_JOURNEY_VIDEO_SCENE_MIN_SEC`. Implemented as `[0:v]split=N → trim+setpts → concat` in `filter_complex`; voice clips are then `adelay`'d to the cumulative output starts and `amix`'d. The lower-third subtitles are re-timed against the same cumulative output timeline. When **`0`**, falls back to the legacy uniform path (`UX_JOURNEY_VIDEO_SLOWDOWN_FACTOR` × `UX_JOURNEY_VIDEO_COMPOUND_SLOWMO`). |
