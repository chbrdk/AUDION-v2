...
if TYPE_CHECKING:
	from checkion_agent.browser.session import BrowserSession, CDPSession, Target
...
class SessionManager:
	...
	async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
		...
		from checkion_agent.browser.session import Target
		...
		from checkion_agent.browser.session import CDPSession
	...
	async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
		...
		if target_fully_removed:
			if target_type in ('page', 'tab'):
				from checkion_agent.browser.events import TabClosedEvent
	...
	async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
		...
		try:
			...
			if page_targets:
				...
				from checkion_agent.browser.events import TabCreatedEvent
			...
			if new_session:
				...
				from checkion_agent.browser.events import AgentFocusChangedEvent
			...
			for _ in range(20):
				...
				if fallback_session:
					...
					from checkion_agent.browser.events import AgentFocusChangedEvent, TabCreatedEvent
