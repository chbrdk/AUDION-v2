...
**Horizontal (≤1024px):** `msqdx-glass-section-nav--horizontal` — `__dock-shell` is a swipeable row (`overflow-x: auto`, `scroll-snap-type: x`). Active tab uses `SECTION_NAV_HORIZONTAL_ACTIVE_CORNER_STYLES` (`bottomLeft` / `bottomRight`: `rounded`) so the black strip rounds into the workspace below. Rail shell uses `SECTION_NAV_HORIZONTAL_DOCK_CORNER_STYLES` (flat bottom). Active section scrolls into view via `scrollIntoView({ inline: "center" })`.
