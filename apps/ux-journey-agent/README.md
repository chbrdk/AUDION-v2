...
| `UX_JOURNEY_OPENAI_MODEL` | no | OpenAI model used as primary (when `OPENAI_API_KEY` is present and Anthropic is not) or as cross-provider fallback (default **`gpt-5.4-mini`**, released 2026-03-17 — ~2× faster / ~6× cheaper than `gpt-5.4` full while still good on browser-use's strict structured-output schema). Override with `gpt-5.4-nano` for cheaper recovery (only OK if Claude rarely fails) or `gpt-5.4` / `gpt-5.5` for higher-quality recovery. |
| `UX_JOURNEY_LLM_FALLBACK` | no | When **`1`** (default), and both `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are configured, the agent passes the **other** provider as `fallback_llm` to browser-use. Recent browser-use versions retry on the fallback for HTTP errors *and* AgentOutput validation failures (e.g. `action` returned as JSON string instead of a list). Set to `0` to disable. The agent prints a `primary=… fallback_llm=…` line at the start of every run so you can verify wiring in the logs. |
